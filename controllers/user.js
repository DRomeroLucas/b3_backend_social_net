...
export const register = async (req, res) => {
    try {
        ...
        // Encriptar la contraseña y guardar en hashedPassword
...
export const login = async (req, res) => {
    try {
        ...
        //  Párametros del body (enviados en la petición)
        let params = req.body;
...
// Método para mostar el perfil
export const profile = async (req, res) => {
    try {
        // Obtener el ID del usuario desde los parámetros de la URL
        const userId = req.params.id;
    
        // Validar si el ID está disponible
        if(!req.user || !register.user.userId){
            return res.status(401).send({
                status: "success",
        ...
        const userProfile = await User.findById(userId).select('-password -role -email -__v'); // La parte final evita mostrar dichos datos

        // Verficar si el usuario buscado no existe
        if(!userProfile) {
            return res.status(401).send({
                status: "success",
                message: "Usuario no autenticado"
...
export const updateUser = async (req, res) => {
    try {
      ...
      // Buscar y actualizar el usuario en Mongodb
  ...
  export const avatar = async (req, res) => {
    try {
      ...
      // Buscar el usuario en la BD para obtener la URL de Cloudinary
